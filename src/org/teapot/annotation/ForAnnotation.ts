...
export default class ForAnnotation extends Annotation {
    ...
    private renderNormal(object: ObjectField, parent: Node, scope: Scope, next: Renderable): Unhandled<TemplateRenderException, Node> {
        ...
        while (true) {
            let condition: Unhandled<InvalidViewAccessException, PrimitiveField> = this.getLinkCondition().getPrimitveField(newScope);
            ...
            let increment: Unhandled<InvalidViewAccessException, PrimitiveField> = this.getLinkIncrement().getPrimitveField(newScope);
